...
DATA_DIR = os.path.join("data", "combine")
DATA_NAMES = ["VOC", "COCO"]
MATTING_IMG_DIRS = {
    "VOC": os.path.join(DATA_DIR, "2matting_img", "testset", "VOC_library"),
    "COCO": os.path.join(DATA_DIR, "2matting_img", "testset", "COCO_library"),
}
COMBINE_IMG_DIRS = {
    "VOC": os.path.join(DATA_DIR, "3combine_img", "VOC_20"),
    "COCO": os.path.join(DATA_DIR, "3combine_img", "COCO_80"),
}
SELECTED_OI_IMG_DIRS = {
    "VOC": os.path.join(DATA_DIR, "3oi_img", "VOC_20"),
    "COCO": os.path.join(DATA_DIR, "3oi_img", "COCO_80"),
}
MODELS = {
    "VOC": ["msrn", "mlgcn", "asl"],
    "COCO": ["msrn", "mlgcn","asl"],
...
COMBINE_METHOD = "random"
VERSION = f"v6_{COMBINE_METHOD}_{'_'.join([str(i) for i in BACKGROUND_COLOR])}_s{1 if DO_SCALE else 0}a{1 if DO_ANGLE else 0}"
...
def get_background(size_x, size_y):
    ...
    # background_img[:, :, 0:3].fill(BACKGROUND_GRAY)
    # background_img[:, :, 3].fill(BACKGROUND_ALPHA)
    for i in range(4):
...
def combine(output_dir, combs, select_num, scale_range, overlap_range, final_size, do_scale, do_angle):
    """
    combine images
    ...
    for comb in combs:
        ...
        if len(centers) == 0:
            ...
            while True:
                ...
                if id not in combined_imgs:
                    break
            combined_imgs[id] = final_img
    ...
    sample_and_save(output_dir, combined_imgs, labels, select_num, components, components_labels)
    return


def sample_and_save(output_dir, combined_imgs, labels, select_num, components=None, components_labels=None):
    """
    sample images

    :param output_dir: output directory
    :param combined_imgs: combined images
    :param labels: labels
    :param select_num: number of selected images
    ...
    """
    # sample
    if len(combined_imgs) > select_num:
        distances = {id: defaultdict(dict) for id in combined_imgs.keys()}
        max_distance = 0
        max_distance_ids = []
        for id1, id2 in combinations(combined_imgs.keys(), 2):
            # id1, id2 = id1, id2 if id1 < id2 else id2, id1
            distance = compare(combined_imgs[id1], combined_imgs[id2])
        ...
        selected_ids = combined_imgs.keys()

    # save images
    for id in selected_ids:
        image = combined_imgs[id]
        # id = '%05d' % random.randint(10000, 99999)
...
def img_combine(
    ...
    :param output_dir: directory of combined images
    ...
    array["img"] = array.progress_apply(
        ...
        axis=1,
    )  # select sample_times*num combinations and than sample num combined images
    ...
    # combine, sample & save
    # print("Combining...")
    # array.swifter.apply(lambda x: combine(
    #     output_dir, x["img"], num, scale_range, overlap_range, final_size, do_scale, do_angle, combine_method
    # ), axis=1)
    tqdm.pandas(
    ...
    print(f"\ncombined, time:{end - start}")


if __name__ == "__main__":
    thread_pool = ThreadPoolExecutor(max_workers=16)
    for dataname in DATA_NAMES[1:]:
        matting_img_dir = MATTING_IMG_DIRS[dataname]
        combine_img_dir = COMBINE_IMG_DIRS[dataname]
        classes = combine_img_dir.split('_')[-1]
        tasks = [
        ...
        for task in tasks:
            ...
            for i, file in enumerate(files):
                ...
                for model in MODELS[dataname]:
                    ...
                    output_dir = os.path.join(f"{combine_img_dir}_{VERSION}", model, f"{task}_No{i+1}")
                    thread_pool.submit(
                        img_combine,
                    ...
                    # end = time.process_time()
                    # print(f"task \"{model} {task}\" of {matting_img_dir} done, total time: {end-start}s")
                    # with open("times.txt", "a") as f:
                    #     f.write(f"{model} {task} of {matting_img_dir}: total time: {end-start}s\n")
        # with open("times.txt", "a") as f:
        #     f.write("\n")
