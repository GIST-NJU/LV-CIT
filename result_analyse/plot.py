...
data_root = os.path.join(
    "..", "data", "combine", "5res_analyse", "ca_gens"
)
ca_dir = os.path.join(
    "..", "data", "combine", "1covering_array"
)
        ...
        for n in ns:
            for tau in [1, 2]:
                ...
                for i in range(3):
                    ...
                    if not len(y):
                        continue
                        # ax.plot(x, [None, None, None, None, None], label=methdos[i], marker=markers[i], color=colors[i], linestyle=line_styles[i])
                    print(x)
                    print(y)
                    ax.plot(x, y, label=methdos[i], marker=markers[i], color=colors[i], linestyle=line_styles[i])
                    # for a, b in zip(x, y):
                    #     plt.text(a, b * 1.1, "{:.2f}".format(myround(b, 2)).rstrip("0").rstrip("."), ha='center', va='bottom', fontsize=10)
